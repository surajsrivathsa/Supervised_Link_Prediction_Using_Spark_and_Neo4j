...
class Step2_Linkexistence(spark: SparkSession) {
  ...
  def generateSamples( graph: GraphFrame, const:constantsFile) : Unit =
  {
    ...
    var authortuplesWithNoLinksInAllYears = spark.sql("select t2.authorId1 as src, t2.authorId2 as dst, t1.year as year from global_temp.t2 as t2 left outer join global_temp.t1 as t1 on t1.src = t2.authorId1 and t1.dst = t2.authorId2 where t1.src is null and t1.dst is null");
    ...
    var authortuplesof_Year3_Notin_other_Years = spark.sql("select e3.src, e3.dst, e3.year   from global_temp.e3 as e3 left outer join " +
    ...
    var authortuplesof_Year2_Notin_Year1 = spark.sql("select e2.src, e2.dst, e2.year from global_temp.e2 as e2 left outer join " +
      "global_temp.e1 as e1 on e2.src = e1.src and e2.dst = e1.dst " +
      "where e1.src is null and e1.dst is null");

    var authortuplesofboth_Year2_and_Year1 = spark.sql("select e2.src, e2.dst, e2.year  from global_temp.e2 as e2 inner join " +
    "global_temp.e1 as e1 on e2.src = e1.src and e2.dst = e1.dst ");

    var authortuplesofboth_Year3_and_others = spark.sql("select  e3.src, e3.dst, e3.year from global_temp.e3 as e3 inner join " +
    ...
    var authortuplesof_Year1_and_2_Notin_Year3 = spark.sql("select e0.src, e0.dst, e0.year   from global_temp.e3 as e3 right outer join " +
      "(select * from global_temp.e2 union select * from global_temp.e1) as e0 " +
    ...
    var linkexists_samples_year2 = edges_year2.select("src", "dst", "year").withColumn("linkexists", lit(1)).orderBy(rand()).limit(1000000);
    linknotexists_samples_year2.union(linkexists_samples_year2).write.option("sep", "|").mode("overwrite").csv(const.dstFolderPath+const.s2f_training_FileName);


    var linkexists_samples_year3 = edges_year3.select("src", "dst", "year").withColumn("linkexists", lit(1)).orderBy(rand()).limit(1000000);
    ...
    linkexists_samples_year3.union(link_notexists_year3).write.option("sep", "|").mode("overwrite").csv(const.dstFolderPath+const.s2f_validation_FileName)
